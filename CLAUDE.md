...
- **Saplings** (`grants` table): one-use founding codes minted by founders (Workshop →
  SAPLINGS, ONE per fort per day — D1-clocked from the last grown, ≤5 unused; a NEWBORN
  fort mints nothing its first day (roots exempt via parent_fort IS NULL), so founding
  chains cost a real day per generation). Token shown exactly once, stored hashed. `POST /api/found` consumes
